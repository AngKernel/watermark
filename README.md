...
本分支将旧 roadhog / webpack / dva 构建链迁移为 **Vite 8 + React 19**，支持 **Node.js 24.x、npm 11+**，不需要降级 Node，也不需要 `--openssl-legacy-provider` 或 `--legacy-peer-deps`。
...
先保存本地修改，再切换迁移分支。仓库已提交新 `package-lock.json`，请保留它。`npm ci` 会清理现有 `node_modules` 并按锁文件安装，不要继续使用旧的 `yarn.lock` 或从旧目录复制依赖。

```sh
git fetch origin
git switch chore/node24-vite
npm ci
npm start
```
